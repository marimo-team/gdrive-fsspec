...
def test_changes_sync_baselines_token_at_construction() -> None:
    ...
    with (
        mock.patch(target, return_value=mock.Mock()),
        mock.patch("gdrive_fsspec.core.build"),
        mock.patch.object(
...
def test_changes_sync_disabled_makes_no_baseline_call() -> None:
    ...
    with (
        mock.patch(target, return_value=mock.Mock()),
        mock.patch("gdrive_fsspec.core.build"),
        mock.patch.object(GoogleDriveFileSystem, "_get_start_page_token") as baseline,
    ):
        fs = GoogleDriveFileSystem(
            token="service_account",
            creds={"type": "service_account"},
...
def test_init_validates_root_file_id() -> None:
    ...
    with (
        mock.patch(target, return_value=mock.Mock()),
        mock.patch("gdrive_fsspec.core.build"),
        mock.patch.object(GoogleDriveFileSystem, "_validate_root_file_id") as validate,
    ):
        GoogleDriveFileSystem(
            token="service_account",
            creds={"type": "service_account"},
...
def test_connect_browser_removes_cache_before_cache_connect() -> None:
    ...
    with (
        mock.patch("gdrive_fsspec.core.os.remove") as remove,
        mock.patch.object(fs, "_connect_cache", return_value=cred) as cache,
        mock.patch("gdrive_fsspec.core.build"),
    ):
...
def test_connect_browser_ignores_missing_cache_file() -> None:
    ...
    with (
        mock.patch("gdrive_fsspec.core.os.remove", side_effect=FileNotFoundError),
        mock.patch.object(fs, "_connect_cache", return_value=mock.Mock()),
        mock.patch("gdrive_fsspec.core.build"),
    ):
        fs.connect(method="browser")
...
def test_connect_cache_passes_auth_kwargs() -> None:
    ...
    with (
        mock.patch(target, return_value=mock.Mock()) as get_creds,
        mock.patch("gdrive_fsspec.core.build"),
    ):
        GoogleDriveFileSystem(
            token="cache",
...
def test_service_account_creds_from_file(tmp_path: Path) -> None:
    ...
    with mock.patch(target) as from_info, mock.patch("gdrive_fsspec.core.build"):
        GoogleDriveFileSystem(
            token="service_account",
            creds=str(cred_file),
            skip_instance_cache=True,
...
def test_connect_owns_authed_http_and_shares_with_build() -> None:
    ...
    with (
        ...
        mock.patch("gdrive_fsspec.core.build") as build,
    ):
